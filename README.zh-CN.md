...
**Spring Boot Starter，集成 aliyun-mps**
...
**aliyun-mps-spring-boot-starter** 是一个面向 使用 aliyun-mps 的应用 的 Spring Boot Starter，用于将 **aliyun-mps** 集成到 Spring Boot 应用中。它提供自动装配、属性绑定与开箱即用的 Bean，使应用以最小配置即可使用 aliyun-mps 的全部能力。

| 维度 | 说明 |
|---|---|
| 类型 | Spring Boot Starter |
| 消费方 | 使用 aliyun-mps 的 Spring Boot 应用 |
| 核心能力 | 自动装配、属性绑定、开箱即用的 aliyun-mps Bean |
| JDK | `17` |
| 坐标 | `io.github.easy4j:aliyun-mps-spring-boot-starter:3.3.x.20260527-SNAPSHOT` |
| 配置前缀 | `aliyun.mps` |
...
| 能力 | 状态 | 说明 |
|---|:---:|---|
| 自动装配 | ✅ 稳定 | 自动注册 aliyun-mps 相关 Bean |
| 属性绑定 | ✅ 稳定 | 绑定 `aliyun.mps.*` 到 `AliyunMpsProperties` |
...
| 属性 | 类型 | 默认值 | 必填 | 说明 | 敏感 |
|---|---|---|:---:|---|:---:|
| `aliyun.mps.enabled` | boolean | `true` | 否 | 是否启用 Starter | 否 |
<!-- 更多属性见下方 -->
...
| 症状 | 诊断 | 解决 |
|---|---|---|
| Bean 未创建 | 查看自动装配报告 | 确认 `aliyun.mps.enabled=true` 与 classpath |
...
[返回顶部](#readme-top) · [问题反馈](https://github.com/easy-4-java/aliyun-mps-spring-boot-starter/issues) · [仓库地址](https://github.com/easy-4-java/aliyun-mps-spring-boot-starter)
